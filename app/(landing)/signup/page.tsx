...
export default function SignupPage() {
  ...
  if (isLoading) {
    return (
      <PageLoader
        ...
        subtext="Registering profile and persona with backend"
        fullScreen={true}
      />
    );
  }
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Call signup passing name, email, password AND userType!
      await signup(name, email, password, userType);
      
      dispatch(
        showNotification({
          title: "Account Created Successfully",
          body: "Please log in to proceed to your workspace.",
          type: "success",
        })
      );

      // Route directly to /login with registered success banner & pre-filled email
  ...
  return (
    <div className="relative min-h-[calc(100vh-4rem)] flex items-center justify-center overflow-hidden bg-background text-foreground py-12 sm:py-16 px-4 sm:px-6 lg:px-8">
      ...
      <div className="relative w-full max-w-5xl grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center z-10">
        ...
        {/* RIGHT COLUMN: GLASSMORPHIC SIGNUP FORM CARD WITH PREMIUM BOX SHADOW */}
            ...
            <form className="mt-7 space-y-5" onSubmit={handleSubmit}>
              <div className="space-y-4">
                <div>
                  ...
                  >
                    <User className="w-3.5 h-3.5 text-foreground" /> Full Name
                  </label>
                  <input
                    id="name"
                    name="name"
                ...
                {/* USER TYPE SELECTION INSIDE SIGNUP FORM */}
                <div className="pt-2">
                  <label className="block text-xs font-extrabold text-foreground uppercase tracking-wider mb-2">
                    Account Persona / User Type
                  </label>
                  <div className="grid grid-cols-2 gap-3">
                    {/* Recruiter Option */}
                    <div
                      onClick={() => setUserType("recruiter")}
                      className={`p-3 rounded-2xl border flex flex-col justify-between transition-all cursor-pointer select-none ${
                        userType === "recruiter"
                    ...
                    >
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        {userType === "recruiter" && <CheckCircle2 className="w-3.5 h-3.5 text-foreground" />}
                      </div>
                      <span className="text-xs font-extrabold text-foreground">Recruiter</span>
                      <span className="text-[10px] text-muted-foreground leading-tight">Evaluate candidate CVs</span>
                    </div>
                    ...
                    >
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        {userType === "candidate" && <CheckCircle2 className="w-3.5 h-3.5 text-foreground" />}
                      </div>
                      <span className="text-xs font-extrabold text-foreground">Job Seeker</span>
                      <span className="text-[10px] text-muted-foreground leading-tight">Audit ATS score</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-2">
                ...
                >
                  ...
                  {loading ? "Creating Account..." : "Create Account & Proceed to Login"}
